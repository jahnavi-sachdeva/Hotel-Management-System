...
public class FrmState extends JInternalFrame {
	JTable jtb;
	JScrollPane jsp;
	JPanel panButtons;
	JButton btnAdd,btnUpdate,btnDelete,btnSearch,btnShowAll;
	String heads[]= {"stateid","statename"};
	Object data[][];
	String condition="";
	FrmState(JDesktopPane jdp){
		super("State Selection",true,true,true,true);
		showTable();
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				FrmEditState cf2=new FrmEditState(FrmState.this,"");
				jdp.add(cf2);
				jdp.setComponentZOrder(cf2, 0);
				jdp.setComponentZOrder(FrmState.this, 1);
			}
		});
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				...
				if(row==-1) {
					JOptionPane.showMessageDialog(FrmState.this, "No record selected");
					return;
				}
				int stateid=Integer.parseInt((String)jtb.getValueAt(row, 0));
				FrmEditState cf2=new FrmEditState(FrmState.this,"select * from state where stateid ="+stateid);
				jdp.add(cf2);
				jdp.setComponentZOrder(cf2, 0);
				jdp.setComponentZOrder(FrmState.this, 1);
			}
		...
		btnDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				...
				dc.executeOtherQuery("delete from state where stateid="+stateid);
				showTable();
			}
		});
		...
		btnSearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				FrmStateSearch cf2=new FrmStateSearch(FrmState.this);
				jdp.add(cf2);
				jdp.setComponentZOrder(cf2, 0);
				jdp.setComponentZOrder(FrmState.this, 1);
			}
	...
	void showTable() {
		try {
			if(jsp!=null)
				remove(jsp);
			DConnection dc=new DConnection();
			ResultSet rst=dc.executeSelectQuery("select count(*) from state"+condition);
			rst.next();
			int n=rst.getInt(1);
			dc.close();
			data=new Object[n][2];
			rst=dc.executeSelectQuery("select * from state "+condition);
